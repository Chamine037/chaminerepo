...
class ConsultarEncomendaProdutoMais extends Component {
    state = {
        nome: '',
        quantidade: '',
        produtos: [],
            ...
            <div className="container">
                ...
                <div className="card z-depth-0 project-summary">
                    <div className="card-content grey-text text-darken-3">
                        <h2 className="grey-text text-darken-3 center">Lista de produtos mais encomendados</h2>
                    </div>
                </div>
                <div className="mostrarclientes-group">
                    <label> <h6>
                        {this.state.produtosOrdenados.map((p) => (
        ...
        for (var i = 0; i < this.state.produtos.length; i++) {
            ...
            for (var j = 0; j < this.state.produtos.length; j++) {
                if(this.state.produtos[j].produto[0].nome == k.produto[0].nome && this.state.produtos[j]._id != k._id){
                    k.quantidade += this.state.produtos[j].quantidade;
                }
            }
            var b = true;
            console.log(k)
...
export default connect(mapStateToProps)(ConsultarEncomendaProdutoMais)